<template>
  <div class="app-container">
    ...
    <el-form :model="queryParams" ref="queryRef" :inline="true" v-show="showSearch" label-width="68px">
      ...
      <el-form-item label="分类来源(0:系统 1:商家)" prop="sourceType">
        <el-select v-model="queryParams.sourceType" placeholder="请选择分类来源(0:系统 1:商家)" clearable>
          <el-option
      ...
      <el-form-item label="审核状态(0:待审核 1:已通过 2:已拒绝)" prop="auditStatus">
        <el-select v-model="queryParams.auditStatus" placeholder="请选择审核状态(0:待审核 1:已通过 2:已拒绝)" clearable>
          <el-option
    ...
    <el-table v-loading="loading" :data="categoryList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="分类来源(0:系统 1:商家)" align="center" prop="sourceType">
        <template #default="scope">
          <dict-tag :options="category_soure_type" :value="scope.row.sourceType"/>
        </template>
      </el-table-column>
      <el-table-column label="审核状态(0:待审核 1:已通过 2:已拒绝)" align="center" prop="auditStatus">
        <template #default="scope">
          <dict-tag :options="category_audit_comment" :value="scope.row.auditStatus"/>
        </template>
      </el-table-column>
...
const { category_soure_type, category_audit_comment } = proxy.useDict('category_soure_type', 'category_audit_comment');
...
const data = reactive({
  ...
  rules: {
    ...
    sourceType: [
      { required: true, message: "分类来源(0:系统 1:商家)不能为空", trigger: "change" }
    ],
    auditStatus: [
      { required: true, message: "审核状态(0:待审核 1:已通过 2:已拒绝)不能为空", trigger: "change" }
    ],
    createTime: [
...
function handleDelete(row) {
  ...
  proxy.$modal.confirm('是否确认删除分类管理编号为"' + _categoryIds + '"的数据项？').then(function() {
